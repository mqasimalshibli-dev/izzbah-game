// Builds `legal.html` FROM the game's own privacy policy and terms.
//
// Run it after any edit to the legal text in `index.html`:
//
//     node about/legal.mjs            # writes legal.html
//     node about/legal.mjs --check    # exits 1 if the file is out of date
//
...
/* ⚠️ The policies live at the SITE ROOT. This URL goes into App Store Connect
   and the Play Console and is quoted in the terms themselves, so it wants to
   be permanent and to not read as a draft — `izzbah.com/legal.html`, not
   `izzbah.com/about/legal.html`. */
const OUT = join(ROOT, "legal.html");
...
<!-- Matches the landing page: the whole of /about/ is still a draft.
...
<meta property="og:url" content="https://izzbah.com/legal.html">
<style>
/* ⚠️ The FULL faces from the game (assets/fonts/), not the landing page's
   subsets in about/fonts/. Those are cut down to the 59 Arabic codepoints
...
<header>
  <div class="wrap">
    ...
    <a class="back" href="about/">رجوع للموقع</a>
  </div>
</header>
...
if (check) {
  ...
  if (current !== page) {
    console.error("legal.html is out of date with the legal text in index.html.\n"
      + "Run:  node about/legal.mjs");
    process.exit(1);
  }
  console.log("legal.html matches the game's legal text.");
} else {
  writeFileSync(OUT, page);
  const words = bodies.join(" ").replace(/<[^>]+>/g, " ").trim().split(/\s+/).length;
  console.log(`wrote legal.html — ${DOCS.length} documents, ${words} words, ${(page.length / 1024).toFixed(1)} KB`);
}
