...
//
//     node preview/clip.mjs
//
// writes preview/shots/turn.webm + turn.mp4 and stamps the page.
//
...
const { v, changed } = stampVersion(join(ROOT, "preview", "index.html"), [webm, mp4], {
  constant: "CLIP_V",
});
{
  const { readFileSync, writeFileSync } = await import("fs");
  const p = join(ROOT, "preview", "index.html");
  const line = `const CLIP_CUES = [${[0, cues.answer, cues.score].map(n => n.toFixed(2)).join(", ")}];`;
  let html = readFileSync(p, "utf8");
  if (!/const CLIP_CUES = \[[^\]]*\];/.test(html))
    console.warn("⚠️  no CLIP_CUES line in preview/index.html — the step buttons will not seek");
  else { writeFileSync(p, html.replace(/const CLIP_CUES = \[[^\]]*\];/, line)); console.log(`cues ${line}`); }
}
console.log(changed ? `stamped preview/index.html with ${v}` : `version unchanged (${v})`);
